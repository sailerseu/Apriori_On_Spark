...
public class SApriori_On_Item_K {
	...
	public static void main (String[] args) throws Exception
	{
		...
		JavaRDD<Tuple2<String,String>> words = lines.flatMap(new FlatMapFunction<String,Tuple2<String,String>>() {
		      ...
		      public Iterable<Tuple2<String,String>> call(String s) {
		        ...
		        for(int i_first=1;i_first<length_of_document;i_first++)//the first is documentid
		        {
		        	/*int pre_length_hs=hs.size();
		        	hs.add(terms[i_first]);
		        	int aft_length_hs=hs.size();
		        	if(pre_length_hs<aft_length_hs)// in order to ensure that all words appear only once
		        	{*/
		        		terms_in_document.add(new Tuple2<String,String>(terms[i_first],terms[0]));
		        	/*}*/
		        }
		...
		*/
			...
			public Iterable<Tuple2<String,String>> call(Tuple2<String,String> t)
			{
				ArrayList<Tuple2<String,String>> term_doc_commbinition_inner=new ArrayList<Tuple2<String,String>>();
				 ...
				 //if(length_of_doc_list>=threshold){
					 for(int d_first=0;d_first<length_of_doc_list;d_first++)
					 {
						 
						 
						 term_doc_commbinition_inner.add(new Tuple2<String,String>(docs[d_first],t._1));
		...
		JavaPairRDD<String,String> docs_terms=term_doc_list.reduceByKey(new Function2<String,String,String>(){
			...
			public String call(String s1,String s2)
			{
				...
				int p_first=0;
				for(;p_first<str_list_length;p_first++)
				{
					if(str_list[p_first].equalsIgnoreCase(s2))
					{
						//return s1;
						break;
					}
				}
				if(p_first==str_list_length)
				{
					s1+="|"+s2;
				}
				return s1;
		...
		JavaRDD<Tuple2<String,String>> terms_docs_second=docs_terms.flatMap(new FlatMapFunction<Tuple2<String,String>,Tuple2<String,String>>(){
			...
			public Iterable<Tuple2<String,String>> call(Tuple2<String,String> t)
			{
				...
				for(int p_second=0;p_second<second_terms_length;p_second++)
				{
					String[] tmp_itemset=second_terms[p_second].split(" ");
					for(int s=0;s<tmp_length_of_itemset;s++)
					{
						unique_terms.add(tmp_itemset[s]);
					}
				}
				...
				if(unique_terms.size()>=iteration)
				{
					//if(second_terms_length>=2){
						...
						{
							for(int d_second=j+1;d_second<second_terms_length;d_second++)
							{
								if(share(second_terms[d_second],second_terms[j]))
								{
									//terms_docs_second_inner.add(new Tuple2<String,String>(second_terms[j],t._1));
									terms_docs_second_inner.add(new Tuple2<String,String>(term_combination(second_terms[j].toString(),second_terms[d_second].toString()).trim(),t._1));
								}
							}
		...
		terms_docs=terms_docs_second_pair.reduceByKey(new Function2<String,String,String>(){
			...
			public String call(String s1,String s2)
			{
				...
				int p_third=0;
				for(;p_third<doc_list_length;p_third++)
				{
					if(doc_list[p_third].equalsIgnoreCase(s2))
					{
						//return s1;
						break;
					}
				}
				if(p_third==doc_list_length)
				{
					s1+=","+s2;
				}
				return s1;
		...
		//sc.close();
		//System.out.println("长度是多少---"+terms_docs_filtered.count());
